...
let renderGlasses=()=>{
    ...
    dataGlasses.forEach((item)=>{
        let content=/*html*/ `
        <div class="col-4">
            <img  src="${item.virtualImg}"  onclick="changeGlass('${item.id}')"/>
        </div>
         `;
         contentHTML+=content;
    });
...
var changeGlass = (id) => {
    let indexGlass = dataGlasses.find((index) => index.id == id);
    document.getElementById('avatar').innerHTML = /*html*/ `
                                                          <img src="${indexGlass.virtualImg}" alt="" />
                                                        `;
    document.getElementById('glassesInfo').style.display = 'block';
    document.getElementById('glassesInfo').innerHTML = /*html*/ `
                                                                  <h5>${indexGlass.name} - ${indexGlass.brand} (${indexGlass.color})</h5>
                                                                  <p>${indexGlass.price} Stocking</p>
                                                                  <p>${indexGlass.description}</p>
                                                                `;
  };
  var before = () => {
